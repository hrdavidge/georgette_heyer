...
labels = [str('Some Similarities ' + str(int(round(some))) + '%'), str('No Similarities ' + str(int(round(no))) + '%'), str('Not Read Biography ' + str(int(round(not_read))) + '%')]
...
# create and save a pit chart of the data of just people who have read a biography
plt.figure()
tot = float(len(df.loc[df['similarity_heyer'] == 'Some Similarities'])) + float(len(df.loc[df['similarity_heyer'] == 'No Similarities']))
some = float(len(df.loc[df['similarity_heyer'] == 'Some Similarities'])) / tot * 100.0
no = float(len(df.loc[df['similarity_heyer'] == 'No Similarities'])) / tot * 100.0
sizes = [some, no]
labels = [str('Some Similarities ' + str(int(round(some))) + '%'), str('No Similarities ' + str(int(round(no))) + '%')]
colors = ['lightskyblue', 'lightpink']
patches, texts, autotexts = plt.pie(sizes, startangle=90, autopct='%.0f%%', colors=colors)
plt.legend(patches, labels, loc="lower center")
plt.axis('equal')
plt.savefig(os.path.join(save_path + 'similarities_read_pie.png'))
